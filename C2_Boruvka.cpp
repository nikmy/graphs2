...
public:
    ...
    Edge EdgeBegin(Edge e) const {
        return edge_begins_[e];
    }

    Edge EdgeEnd(Edge e) const {
        return edge_ends_[e];
    }
...
Weight MSTWeightBoruvka(const WeightedGraph& g) {
    Weight mst_weight = 0;
    auto dsu = DSU<Vertex>(g.NVertices());
    std::vector<Edge> min_edge(g.NVertices());
    std::vector<bool> used(g.NEdges(), false);
    while (dsu.NSets() > 1) {
        min_edge.assign(g.NVertices(), -1);
        for (Edge e = 0; e < static_cast<Edge>(g.NEdges()); ++e) {
            auto comp_src = dsu.FindSet(g.EdgeBegin(e));
            auto comp_dst = dsu.FindSet(g.EdgeEnd(e));
            if (comp_src != comp_dst) {
                if (min_edge[comp_src] == -1 || g.EdgeWeight(e) < g.EdgeWeight(min_edge[comp_src])) {
                    min_edge[comp_src] = e;
                }
                if (min_edge[comp_dst] == -1 || g.EdgeWeight(e) < g.EdgeWeight(min_edge[comp_dst])) {
                    min_edge[comp_dst] = e;
                }
            }
        }
        for (auto e : min_edge) {
            if (e != -1 && !used[e]) {
                dsu.Union(g.EdgeBegin(e), g.EdgeEnd(e));
